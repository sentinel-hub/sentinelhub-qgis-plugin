...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("S1GRD", "VH-DB", "S1GRD_ASC", ServiceType.WMTS, f"S1GRD_ASC - VH-DB (WMTS, {START_TIME}/{END_TIME}, {PRIORITY}, {CRS})"),
        ("S2L2A", "TRUE-COLOR", "S2L2A", ServiceType.WCS, f"S2L2A - TRUE-COLOR (WCS, {START_TIME}/{END_TIME}, {MAXCC}%, {PRIORITY}, {CRS})"),
        ("S2L1C", "TRUE-COLOR", "S2L1C", ServiceType.WFS, f"S2L1C (WFS, {START_TIME}/{END_TIME}, {MAXCC}%, {PRIORITY}, {CRS})")
    ]
)
...
@pytest.mark.parametrize(
    ...
    [
        ("DEM", "COP_30", "TOPOGRAPHIC-VIS", True, f"COP_30_{LAYER_ID}_{FILENAME_BOX}_{FILENAME_CRS}_logo.{IMAGE_FORMAT}"),
        ("S1GRD", "S1GRD_ASC", "VH-DB", True, f"S1GRD_ASC_{LAYER_ID}_{FILENAME_TIME}_{FILENAME_BOX}_{FILENAME_CRS}_{PRIORITY}_logo.{IMAGE_FORMAT}"),
        ("S2L2A", "S2L2A", "TRUE-COLOR", True, f"S2L2A_{LAYER_ID}_{FILENAME_TIME}_{FILENAME_BOX}_{FILENAME_CRS}_{MAXCC}_{PRIORITY}_logo.{IMAGE_FORMAT}"),
        ("S2L1C", "S2L1C", "TRUE-COLOR", False, f"S2L1C_{LAYER_ID}_{FILENAME_TIME}_{FILENAME_BOX}_{FILENAME_CRS}_{MAXCC}_{PRIORITY}.{IMAGE_FORMAT}")
    ]
)
